// TODO: Обновите сигнатуру `add_ticket`: метод должен принимать `TicketDraft`
//  и возвращать `TicketId`.
//  У каждой заявки должен быть уникальный id, генерируемый `TicketStore`.
//  При необходимости можете изменить поля `TicketStore`.
//
// Также добавьте метод `get`, который принимает `TicketId`
// и возвращает `Option<&Ticket>`.
